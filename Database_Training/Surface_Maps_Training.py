...
class SurfaceMap_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(surface_maps[0], torch.Tensor):
            surface_maps_tensor = torch.cat(surface_maps, dim=0)
            if surface_maps_tensor.shape[0] == self.hours_back*3:
                return surface_maps_tensor
            else:
                raise ValueError(f'Expected {self.hours_back*3} surface maps, but got {surface_maps_tensor.shape[0]}')
        else:
